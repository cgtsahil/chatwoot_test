...
#  mail_status           :string           default("unread")
#  mail_subject          :string
...
class Message < ApplicationRecord
  ...
  def blocked
    ...
    error.add(:conversation_id, 'Conversation is blocked') if conversation.blocked
  end
  ...
  def execute_after_create_commit_callbacks
    ...
    update_contact_mail_subscription
  end

  def update_contact_mail_subscription
    current_message = self
    return if current_message.message_type != 'incoming' || current_message.inbox.inbox_type != 'Email'

    contact = current_message.conversation.contact
    return if contact['mail_subscription'] != 'subscribed'

    contact.update(mail_subscription: 'active', email_campaigns: [])
  end
