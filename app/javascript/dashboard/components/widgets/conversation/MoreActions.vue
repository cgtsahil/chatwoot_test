<template>
  <div class="flex-container actions--container">
    <woot-button
      ...
      v-tooltip="$t('CONTACT_PANEL.BLOCK_CONTACT')"
      variant="clear"
    ...
    <woot-button
      v-else
      v-tooltip="$t('CONTACT_PANEL.UNBLOCK_CONTACT')"
      variant="clear"
      color-scheme="alert"
      icon="dismiss-circle"
      @click="unblock"
...
import {
  ...
  CMD_BLOCK_CONVERSATION,
  CMD_UNBLOCK_CONVERSATION,
...
export default {
  ...
  mounted() {
    ...
    bus.$on(CMD_BLOCK_CONVERSATION, this.block);
    bus.$on(CMD_UNBLOCK_CONVERSATION, this.unblock);
  ...
  methods: {
    block() {
      this.$store.dispatch('blockConversation', this.currentChat.id);
      this.showAlert(this.$t('CONTACT_PANEL.CONTACT_BLOCKED'));
    },
    unblock() {
      this.$store.dispatch('unblockConversation', this.currentChat.id);
      this.showAlert(this.$t('CONTACT_PANEL.CONTACT_UNBLOCKED'));
    },
    mute() {
