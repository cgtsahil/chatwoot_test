...
const actions = {
  ...
  blockConversation: async ({ commit }, conversationId) => {
    try {
      await ConversationApi.block(conversationId);
      commit(types.BLOCK_CONVERSATION);
    } catch (error) {
      //
    }
  },

  unblockConversation: async ({ commit }, conversationId) => {
    try {
      await ConversationApi.unblock(conversationId);
      commit(types.UNBLOCK_CONVERSATION);
    } catch (error) {
      //
    }
  },
