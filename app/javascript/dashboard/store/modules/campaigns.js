...
export const getters = {
  ...
  getMailCampaigns: _state => campaignType => {
    return _state.records.filter(
      record =>
        record.campaign_type === campaignType &&
        record.custom_attributes.action === 1
    );
  },
  getMailReplies: _state => campaignType => {
    return _state.records.filter(
      record =>
        record.campaign_type === campaignType &&
        (record.custom_attributes.action === 2 ||
          record.custom_attributes.action === 3)
    );
  },
